<template>
  <div :style="{ cursor, userSelect}" class="vue-splitter-container clearfix" @mouseup="onMouseUp" @mousemove="onMouseMove">
    ...
    <pane class="splitter-pane splitter-paneL" :split="split" :style="{ [type]: percent+'%', [`min-${type}`]: leftMinPixel + 'px'}">
      <slot name="paneL"></slot>
    </pane>

    <resizer :className="className" :style="{ [resizeType]: resizerStyle}" :split="split" @mousedown.native="onMouseDown" @click.native="onClick"></resizer>
...
<script>
  ...
  export default {
    ...
    mounted() {
      this.targetOffset = this.split === 'vertical' ? this.$el.offsetWidth : this.$el.offsetHeight
    },
    computed: {
      userSelect() {
      ...
      resizerStyle() {
        const px = this.percent * this.targetOffset / 100
        if (px < this.leftMinPixel) {
          return this.leftMinPixel
        } else {
          return px
        }
      }
    ...
    methods: {
      ...
      onMouseMove(e) {
        ...
        if (this.active) {
          ...
          const percent = Math.floor(((currentPage - offset) / this.targetOffset) * 10000) / 100
